...
export default class NostrRequest {
	...
	private relaySubs: SuperMap<Relay, ZenObservable.Subscription[]> =
		new SuperMap(() => []);

	constructor(relayUrls: Iterable<string>, timeout?: number) {
		this.relays = new Set(
			Array.from(relayUrls).map((url) => relayPoolService.requestRelay(url)),
		);
	...
	handleEvent(message: IncomingEvent) {
		if (
			...
			message[1] === this.id &&
			!this.seenEvents.has(message[2].id)
		) {
