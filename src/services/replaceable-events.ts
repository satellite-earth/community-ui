...
class ReplaceableEventsService {
	...
	private readFromCacheThrottle = _throttle(
		this.readFromCache,
		READ_CACHE_BATCH_TIME,
	);
	private async readFromCache() {
		...
		if (events.length > 0)
			this.dbLog(`Read ${events.length} events from database`);
	}
	...
	private writeToCacheThrottle = _throttle(
		this.writeToCache,
		WRITE_CACHE_BATCH_TIME,
	);
